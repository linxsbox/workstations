...
<template>
  <section class="rss-list flex-1 h-full rounded-md overflow-hidden" :style="getThemeColor(props.data.theme)">
    <NScrollbar class="rss-list-box flex flex-col gap-3 h-full">
      <header class="rss-list-header sticky top-0 flex-none flex gap-2 items-center p-4 z-10">
        <figure class="flex-none h-12">
          <picture>
            <img class="size-12 rounded-lg" :src="props.data.image" v-if="props.data.image" />
          </picture>
        </figure>
        <div class="flex-auto">
          <div class="flex justify-between items-center">
            <a class="title text-lg font-bold" :href="props.data.sourceUrl" target="_blank" rel="noopener noreferrer">
              {{ props.data.title }}
            </a>
      ...
      <div class="flex flex-col gap-3 p-4">
        <RssCardView v-for="item in props.data.list" :data="item" :key="item.link" />
      </div>
    </NScrollbar>
  </section>
</template>
